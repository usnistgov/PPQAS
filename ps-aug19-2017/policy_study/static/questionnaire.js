...
$().ready(function () {
	...
	$('input[type=number]').change(function(obj) {$(obj.target).valid();  });

	$('input[type=checkbox]').on('change', function (e) {
		var maxlength = $(this).prop("maxlength");
		//alert(JSON.stringify($(this).prop("maxlength")) + " " + e.currentTarget['maxlength'] + " " + $('input[name="' +e.currentTarget.name+'"]:checked').length);
		if (maxlength == -1){
			return;
		}

		if ($('input[name="' +e.currentTarget.name + '"]:checked').length >maxlength) {
			$('label[for="' + e.currentTarget.name + '"]').show();
			//$(this).prop('checked', false);

			//alert("allowed only " + maxlength);
		}else if ($('input[name="' +e.currentTarget.name + '"]:checked').length <= maxlength) {
			$('label[for="' + e.currentTarget.name + '"]').hide();
		}
	});
